use super::*;
    ...
    fn reflection_test(){
        ...
        rt.block_on(async {
            ...
            let spheres  = vec!{
                // Sphere::new_with_texture(Vec3 {x: -0.5, y: 0.0, z: -1.0,}, 0.5, Some(Vec3::new(1.0, 1.0, 1.0)), Some(SCATTER_M), ImageTexture:: from_path("assets/default.png").expect("image not found")),
                Sphere::new(Vec3 {x: 0.520, y: 0.0, z: -1.0,}, 0.45, Some(Vec3::new(1.0, 0.9 , 1.0)), Some(METALLIC_M)),
                Sphere::new_with_texture(Vec3 {x: -1000.4, y: 0.0, z:  -1.0}, 1000.0, Some(Vec3::new(0.60, 0.5, 1.0)), Some(SCATTER_M), ImageTexture::from_color(Vec3{x: 1.0, y: 1.0, z: 1.0}.to_rgb()))//ImageTexture::from_path("assets/earthmap.jpg").expect("image not found")),
            };
            let scene = Scene::new(spheres);
            let viewport = Viewport::new_from_res(800, 600 , samples, 10, 2.0, None, None, None, None, Some("texture test".to_string()), None);
